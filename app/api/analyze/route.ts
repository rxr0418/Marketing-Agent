...
5. Call evaluate_signal for the notable items/groups first, then identify_need for each concrete need, then propose_campaign once per need you're proposing a campaign for. Do not call any tool after your last propose_campaign call.`;
...
const tools: Anthropic.Tool[] = [
  ...
  },
  {
    ...
    description:
      "Propose ONE Prime Day campaign focused on exactly one identified need. Call this once per need you're confident about (high or medium confidence only) — never combine multiple needs into one campaign, and never call this for a low-confidence need.",
    input_schema: {
      type: "object",
      properties: {
...
export async function POST(req: NextRequest) {
  ...
  try {
      ...
      for (const block of resp.content) {
        if (block.type === "text" && block.text.trim()) {
          ...
          if (block.name === "evaluate_signal") {
            ...
            campaigns.push(input as unknown as Campaign);
          }
    ...
    trace.push({
      ...
      text:
        campaigns.length === 1
          ? "Proposed 1 single-theme campaign."
          : `Proposed ${campaigns.length} single-theme campaigns — kept separate rather than blended into one headline.`,
    });
